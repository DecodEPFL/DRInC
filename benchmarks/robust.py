...
case realization of the noise within its support. Finding the worst case cost
is a non-convex, NP hard problem, so we only look at the worst training sample.
The constraints are however enforces for all samples in the support.
...
def synthesize_robust(sys: LinearSystem, t_fir: int, feasible_set: Polytope,
    ...
    This closure solves the following optimization problem:
        min_{phi} max_{xi~P_empirical} (xi^T Phi^T W Phi xi)
    ...
    def mkrob(xis, weights=None):
        ...
        # Worst case so for each sample, cost > xi.T @ phi^T @ W @ phi @ xi
        for xi in xis.T:
            c_xi += [cp.bmat([[cost, xi[None, :] @ phi.T],
                              [phi @ xi[:, None], weights]]) >> 0]
